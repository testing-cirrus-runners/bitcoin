...
static RPCHelpMan getpeerinfo()
{
    return RPCHelpMan{
        ...
        [&](const RPCHelpMan& self, const JSONRPCRequest& request) -> UniValue
{
    const NodeId filter_id = request.params[0].isNull() ? -1 : request.params[0].getInt<NodeId>();
    ...
    for (const CNodeStats& stats : vstats) {
        if (filter_id > -1 && stats.nodeid != filter_id) continue;
        ...
        if (filter_id > -1 && stats.nodeid == filter_id) break;
    }
